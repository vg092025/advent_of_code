...
count_paper_rolls(RowList) ->
    ...
    lists:foldl(fun(R, {Ctr, Indices}) ->
        lists:foldl(fun(C, {CtrAcc, IndexAcc} = Acc) ->
            ...
            case is_accessible(CellValue, R, C, RowList, CacheData, RowsCtr, ColCtr) of
                true -> {CtrAcc + 1, [ {R, C} | IndexAcc ]};
                false -> Acc
            end
        end, {Ctr, Indices}, lists:seq(1, ColCtr))
...
process_input() ->
    ...
    PaperRolls = process(RowList, 0),
    io:format("Paper Rolls accessible: ~p~n", [PaperRolls]).

process(RowList, RollCtr) ->
    print_rowlist(RowList),
    io:format("Ctr so far: ~p~n", [RollCtr]),

    case count_paper_rolls(RowList) of
        {Ctr, []} ->
            io:format("No more accessible positions. Total Paper Rolls needed: ~p~n", [Ctr]),
            RollCtr;
        {Ctr, Indices} ->
            NewRollList = update_rows(RowList, Indices),
            process(NewRollList, RollCtr + Ctr)
    end.

update_rows(RowList, Indices) ->
    lists:foldl(fun({R, C}, AccRowList) ->
        Row = lists:nth(R, AccRowList),
        NewRow = lists:sublist(Row, C - 1) ++ [$.] ++ lists:nthtail(C, Row),
        lists:sublist(AccRowList, R - 1) ++ [NewRow] ++ lists:nthtail(R, AccRowList)
    end, RowList, Indices).
